		     ...
		     char *buf)			// OUT: where to copy data
{
   ...
   for (frag = skb_shinfo(skb)->frags; frag != skb_shinfo(skb)->frags + skb_shinfo(skb)->nr_frags; frag++) {

		if (frag->bv_len > 0) {
			unsigned int tmpCsum;
			const void *vaddr;

			vaddr = kmap(frag->bv_page);
			tmpCsum = csum_and_copy_to_user(vaddr + frag->bv_offset, curr, frag->bv_len, 0, &err);
			kunmap(frag->bv_page);
		if (err)
			return err;
