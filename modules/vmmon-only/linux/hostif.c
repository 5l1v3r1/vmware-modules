			...
			PageCnt numPages)	// IN: number of pages to free
{
  ...
  for (cnt = 0; cnt < numPages; cnt++)
	...
	{
	  printk (KERN_DEBUG "Attempted to free unallocated MPN %016" FMT64
		  "X\n", mpns[cnt]);
	  return -EINVAL;
	}
...
HostIFFreeVMHost (VMHost * vmhost)	// IN:
{
  ASSERT (vmhost->lockedPages == NULL &&
	  ...
	  vmhost->crosspagePagesCount == 0 && !vmhost->hostAPICIsMapped);
...
HostIFAllocVMHost (uint32 numVCPUs)	// IN:
{
  ...
  if ((vmhost->crosspagePages =
      ...
      && (vmhost->vcpuSemaTask =
	  Vmx86_Calloc (numVCPUs, sizeof *vmhost->vcpuSemaTask,
			TRUE)) != NULL)
    {
      return vmhost;
    }
	  ...
	  if (entryPtr == NULL)
	      ...
	      else
		{
		  printk (KERN_DEBUG "Page %p disappeared from %s(%u)... "
			  ...
			  current->comm, current->pid, entryPtr->mpn);
		  *mpn = entryPtr->mpn;
		}
	    }
	}
